...
if __name__ == '__main__':
  ...
  # build adIndexList
  adIndexList = random.sample(range(len(goodsAdsList)), len(emailIndexList))
  ...
  for emailIndex, adIndex in zip(emailIndexList, adIndexList):
    emailAccountFrag = emailList[emailIndex]['EMAIL'].split("@") 
    emailList[emailIndex]['EMAIL'] = emailAccountFrag[0] + "+" + personalInformation.generate_name().replace(' ','').lower() + "@" + emailAccountFrag[1]    
    ...
    """
    categoryList = ["cellphone", "autoparts"]
    for count in range(NUM_ADS_PER_ACCOUNT):
      while True:
        adIndex = random.randint(0, len(goodsAdsList)-1)
        #print categoryList, convertedAdIndex        
        if goodsAdsList[adIndex]["CATEGORY"] in categoryList:
          categoryList.remove(goodsAdsList[adIndex]["CATEGORY"])
          break
      #endwhile
        
      print '    Ad %d(%d), %s, %s' \
            % (count, adIndex, goodsAdsList[adIndex]['TITLE'], goodsAdsList[adIndex]['CATEGORY'])
